...
`flet_audio` is imported lazily, inside __init__, and failure is caught:
a packaged build (Android/iOS, or any run mode that bundles its own
requirements rather than using this repo's venv directly) can, in
practice, be built from a snapshot that predates this dependency being
added to requirements.txt -- a missing *optional* package like this must
never take the whole app down at import time. When it's missing,
SoundPlayerFlet.play() just silently no-ops instead of crashing.
"""
...
class SoundPlayerFlet:
    def __init__(self, page: ft.Page) -> None:
        self._page = page
        self._audio_controls: dict = {}

        try:
            import flet_audio as fa
        except ImportError:
            return  # sound is unavailable this session; play() below no-ops

        self._audio_controls = {name: fa.Audio(src=f"sounds/{name}.wav") for name in SOUND_NAMES}
        page.overlay.extend(self._audio_controls.values())

    def play(self, name: str, settings: Settings) -> None:
        """Fire-and-forget. No-ops quietly if sound is disabled, `flet_audio`
        isn't available in this build, or `name` isn't a known sound -- a
        missing/disabled sound should never be a reason a lesson can't be
        completed."""
