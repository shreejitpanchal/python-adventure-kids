...
def make_code_editor(initial_code: str = "", height: int = 220) -> ft.TextField:
    return ft.TextField(
        value=initial_code,
        multiline=True,
        min_lines=6,
        max_lines=20,
        height=height,
        text_style=ft.TextStyle(font_family=EDITOR_FONT_FAMILY, size=15, color=EDITOR_TEXT_COLOR),
        bgcolor=EDITOR_BGCOLOR,
        border_color="#3A3A4E",
    )


def make_read_only_code_block(code: str) -> ft.Control:
    """A non-editable, monospace code block for showing examples."""
    return ft.Container(
        content=ft.Text(code, font_family=EDITOR_FONT_FAMILY, size=15, color=EDITOR_TEXT_COLOR, selectable=True),
