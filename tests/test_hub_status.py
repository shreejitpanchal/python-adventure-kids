...
def test_ai_course_status_starts_at_zero_complete(engine, progress):
    from app.engine.categories import AI_COURSE_CATEGORIES

    status = compute_hub_status(engine, progress, Settings())
    total = sum(len(engine.lessons_in_category(category)) for category in AI_COURSE_CATEGORIES)
    assert status.ai_course_status == f"0/{total} lessons complete"


def test_ai_course_status_reflects_real_progress(engine, progress):
    from app.engine.categories import AI_COURSE_CATEGORIES

    all_ids = [
        lesson.id for category in AI_COURSE_CATEGORIES for lesson in engine.lessons_in_category(category)
    ]
    for lesson_id in all_ids[:2]:
        progress.complete_lesson(lesson_id, 3)
    ...
    status = compute_hub_status(engine, progress, Settings())
    total = len(all_ids)
    assert status.ai_course_status == f"2/{total} lessons complete"
